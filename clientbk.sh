...
sleep 2
cat outputclient
openssl pkeyutl -derive -inkey dhkey1.pem -peerkey outputclient -out bob_shared_secret.bin
echo "secret"
...
receive(){
#rm -rf /tmp/output
cat /tmp/output | tr -d '\000'| grep -E '=+'
while (( $? > 0))
do
sleep 1
echo "wait"
...
echo "decoding"
return cat /tmp/output |tr -d '\000' | grep -E '=+' | openssl enc -aes256 -base64-kfile alice_shared_secret.bin -d 2>/dev/null 
#rm -rf /tmp/output
}
while true
do
